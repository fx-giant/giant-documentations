...
| isOverrideOrdering | Boolean | Flag to indicate whether the form menu ordering can override GIANT default menu.
| applicationOrdering | Integer | Ordering number of the form in form list menu in GIANT menu list. If same ordering number, form title will be used for ordering. Refer [GIANT Menu Ordering](#GIANT-Menu-Ordering) for more details, work together with `isOverrideOrdering`. Without `isOverrideOrdering`, it will stand as normal form menu ordering number after GIANT default menu.
